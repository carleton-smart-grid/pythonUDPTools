...
Function Call | Precondition(s) | Postcondition(s)
--------------|-----------------|-----------------
`generateKey()` | N/A | <ol><li> Returns a 16 byte random key of type `byte string`  </li> </ol>
`encryptAES(data, key)` | <ol><li> data is of type `bytes` </li> <li> Data is 14 bytes long </li> <li> key is of type `bytes` as generated  </li> </ol> | <ol> <li> Returns an encrypted value of type `bytes` that is 16 bytes long </li> </ol>
`decryptAES(data, key)` | <ol><li> data is of type `bytes` </li> <li> Data is 16 bytes long </li> <li> key is of type `bytes` as generated  </li> <li> The key is the same key used with `encryptAES(data, key)` </li> </ol> | <ol><li> Returns an unencrypted value of type `bytes` that is 14 bytes long </li></ol>


## rsatools.py
Contains three functions that are used by `ca.py` and `encryptiontool.py`. `rsatools.generateKey()` which saves both a full key pair and a public key to a specific directory (These paths are within the source code under the constants `KEY_PATH`, and `PUBLIC_KEY_PATH` respectively). The other two functions `data = rsatools.encryptRSA(dat)` and `data = rsatools.decryptRSA(dat)` both do exactly as the name implies. Encrypt takes unencrypted data and returns encrypted data, the other takes encrypted data to return usable data.
...
Function Call | Precondition(s) | Postcondition(s)
--------------|-----------------|-----------------
`generateKey()` | <ol><li> Neither `publickey.bin` nor `key.bin` exist in the target directory</li></ol> | <ol><li> key.bin was created in target directory, containing both the private and public key information</li> <li> publickey.bin was created containing only the public key information of the key pair </li></ol>
`encryptRSA(dat)` | <ol><li> `publickey.bin` i located in the target directory</li> <li> dat is less than or equal to 256 bytes in size </li> <li> dat is of type `bytes` </li></ol> | <ol><li> Returns the dat encrypted with the RSA public key</li> <li> Returned data is of type bytes, 256 bytes in length </li></ol>
`decryptRSA(dat)` | <ol><li> key.bin is in the appropriate directory, containing the appropriate private key </li> <li> dat is of type `bytes` and 256 bytes long </li></ol> | <ol><li> Returns unencrypted data of type bytes</li></ol>

## encryptiontool.py
The `encryptiontool.py` is a simple class that handles the ta's list of current CA AES keys. It does this by keeping a list of CA IPs and their respective AES key. A CA can expire an old key by simply sending a new key.
